...
```env
# Server-only. Never prefix this with NEXT_PUBLIC_ — anything NEXT_PUBLIC_ is
# inlined into the browser bundle and is readable by anyone who loads the site.
# The browser never sees this key; /api/gemini-key mints a short-lived
# ephemeral token from it instead.
GEMINI_API_KEY=your_gemini_api_key_here
NEXT_PUBLIC_SUPABASE_URL=https://your-project.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=your_supabase_anon_key_here
# Server-only. Used by the API routes for server-authoritative writes and for
# the /api/gemini-key rate limiter.
